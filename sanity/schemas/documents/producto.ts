...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      description: 'Puede tener más de uno (ej. Mate y Satinado). Vacío si el cliente no lo especifica.',
    ...
    defineField({
      ...
      group: 'atributos',
      of: [{type: 'string'}],
      options: {list: opciones(TEXTURAS), layout: 'grid'},
      description: 'Puede tener más de una (ej. Rústico y Estructurado). Vacío si el cliente no lo especifica.',
      validation: (rule) => rule.unique(),
    }),
    defineField({
      name: 'uso',
    ...
    defineField({
      ...
      type: 'boolean',
      group: 'tecnico',
      description: 'Bordes cortados a 90° para juntas mínimas. Se muestra como Sí/No; sin definir, la fila se oculta.',
    }),
    defineField({
      name: 'absorcionAgua',
      title: 'Absorción de agua (%)',
      type: 'string',
      group: 'tecnico',
      description: 'Rango del catálogo, p. ej. «3–6». Texto libre porque el dato es un rango, no un número.',
    }),
    defineField({
      name: 'mtsCaja',
